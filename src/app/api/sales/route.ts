...
export async function GET(req: NextRequest) {
  ...
  let qb = supabase.from("sales").select("*", { count: "exact" });

  if (search) {
    qb = qb.or(`data->>'productName'.ilike.%${search}%,data->>'channel'.ilike.%${search}%`);
  }
  ...
  const validation = SaleListSchema.safeParse(body);
...
export async function POST(req: NextRequest) {
  ...
  const body = await parseBody(req, CreateSaleBodySchema);
  ...
  const sale = { id: data.id, ...((data as { data: Record<string, unknown> }).data) };
  const validation = SaleSchema.safeParse(sale);
  if (!validation.success) {
    return errorResponse(500, "SCHEMA_MISMATCH", "Created sale shape invalid", validation.error.flatten());
  }
